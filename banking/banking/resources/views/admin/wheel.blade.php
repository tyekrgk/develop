    ...
    <table class="table table-striped table-bordered td-center" id="data-lists">
      ...
      @foreach($data as $v)
        <tr>
          <th>{{$v['master_id']}}</th>
          <th>{{$v['master_name']}}</th>
          <th>{{$v['master_suffer']}}</th>
          <th>{{$v['master_phone']}}</th>
          <th><img style="width:120px;height:110px;" src="{{$v['master_url']}}"/></th>
          <th>{{$v['master_prove']}}</th>
          <th>{{$v['master_email']}}</th>
          <th>
            @if($v['master_jib'] == 1)
              初级
            @elseif ($v['master_jib'] == 2)
              中级
            @elseif ($v['master_jib'] == 3)
              高级
            @endif
          </th>
          <th><img style="width:120px;height:110px;" src="{{$v['prove_url']}}" alt="" /></th>
          <th>
          @if($v['master_static'] == 0)
            <a class="succ" style="cursor:pointer" data-id="{{$v['master_id']}}">是</a>
          @elseif($v['master_static'] == 1)
            审核通过
            |&nbsp;&nbsp;
            <a style="cursor:pointer;" class="err" data-id="{{$v['master_id']}}">不想通过了</a>
          @endif
          </th>
        </tr>
      @endforeach
...
<script>
  $(function(){
    $(".succ").click(function(){
      ...
      $.ajax({
        type:"get",
        url:"{{url('admin/success')}}",
        data:{'id':name},
        success:function(data){
          if(data == 1){
            alert('审核通过')
            location.href=("{{url('admin/wheel')}}");
          }
        }
      })
    })
