  ...
  <div class="table-responsive">
    <table class="table table-striped table-bordered td-center" id="data-lists">
      <thead>
        ...
        @foreach ($ren as $p)
        <tr class="have_price">
          <th class="price_zhi">{{$p->stock_id}}</th>
          <th>{{$p->stock_name}}</th>
          <th>  <span class='price_jian'>-</span> <span class='price_zhi'> {{$p->stock_price}} </span><span class='price_jia'>+</span> </th>
          <th>{{$p->stock_number}}</th>
          <th>{{$p->stock_sale}}</th>
          <th>{{$p->stock_name}}</th>
          <th><a href="{{URL('/admin/delete').'/'.$p->stock_id}}">删除</a></th>
        </tr>
      @endforeach
      </thead>
      <tbody>  
